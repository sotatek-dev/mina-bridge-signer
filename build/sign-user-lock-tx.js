// src/contract/prove_user_lock.js
import { AccountUpdate, fetchAccount, Field as Field3, Mina as Mina2, PublicKey as PublicKey4, UInt32, UInt64 as UInt642 } from "../node_modules/o1js/dist/node/index.js";
...
// src/contract/prove_user_lock.js
var UserLockBuilder = class extends MinaScBuilder {
  constructor({
    amount,
    address,
    tokenAddress
  ...
  async build() {
    ...
    const tx = await Mina2.transaction(
      ...
      async () => {
        AccountUpdate.fundNewAccount(PublicKey4.fromBase58(this.payload.address), 1);
        await zkApp.lock(UInt642.from(this.payload.amount), Field3.from(1), PublicKey4.fromBase58(this.payload.tokenAddress));
      }
    );
    console.log("proving");
    await tx.prove();
...
// src/sign-user-lock-tx.js
var sign_user_lock = async ({ amount, address, tokenAddress }) => {
  try {
